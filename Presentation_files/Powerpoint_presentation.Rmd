...
- Use a logistic regression to model a binary outcome. 
...
--
  ...
  - Are student who participate in clubs more likely to retain 
Fall to Fall, yes or no?
...
<img src="../figure-html/the_problem.png">
...
```{r, echo = F, message = F, fig.height = 5, fig.width = 6, fig.align = "center"}
...
**An observation's position on the curve is contingent on its properties, and the magnitude of effect varies in percentage points in a non-linear way.**
...
- Built a logistic regression model: which variables are associated with a student passing the Balloon Animal-Making 
...
```{r change-in-log-odds, echo = F, fig.align='center'}
```
...
```{r change-in-log-odds-adjusted-axis, echo = F, fig.align = 'center'}
```
...
.pull-left[
```{r change-in-log-odds-adjusted-axis, echo = F}
```
]
...
```{r odds-ratio-adjusted-axis, echo = F, fig.align = 'center'}
```
...
--
  ...
  1. Odds are expressed as integer ratios ("3-to-1" or "2-to-5") rather than "3" or "0.4”
...
```{r probability-relative-to-some-baseline-and-group-no-arrows, echo = F, fig.align = 'center'}
```
...
```{r probability-relative-to-some-baseline-and-group-with-arrows, echo = F, fig.align = 'center'}
```
...
```{r banana-graph-bare, echo = F, fig.align = 'center'}
```
...
```{r banana-graph-annotated, echo = F, fig.width = 9, fig.align = 'center'}
```
...
```{r banana-graph-multiple, echo = F, fig.align = 'center'}
```
...
```{r extra-passes, message = F, echo = F, fig.align = 'center'}
```
...
```{r extra-passes-by-group, echo = F, fig.align = 'center'}
```
...
```{r potential-passes-by-group, warning = F, echo = F, fig.align = 'center'}
```
