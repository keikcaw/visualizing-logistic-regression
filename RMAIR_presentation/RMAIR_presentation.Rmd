...
```{r probability-relative-to-some-baseline-with-arrows, fig.show = "hide"}
```

```{r probability_baseline_arrows_plot, out.width = "100%"}
prob.baseline.arrows.p = prob.baseline.arrows.p +
  coord_cartesian(xlim = c(0, 1),  ylim = c(0.9, 11.1), clip = "off")
prob.baseline.arrows.p.full = prob.baseline.arrows.p +
  ggtitle(str_wrap(gsub("\\n", " ", prob.baseline.p$labels$title), 50))
prob.baseline.arrows.p.full
```
...
.pull-left[
```{r out.width = "100%", fig.width = 4.4, fig.asp = 1.3}
prob.baseline.arrows.p.half = prob.baseline.arrows.p +
  guides(color = guide_legend(nrow = 3)) +
  theme(legend.position = "bottom")
prob.baseline.arrows.p.half
```
]
